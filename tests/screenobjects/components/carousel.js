...
class Carousel extends Gestures {
  ...
  verifyNthCardContainsText(nthCard, partialText) {
    ...
    const cardText = getTextOfElement(cards[cardNumber])
      .replace(/(?:\r\n|\r|\n)/g, ' ')
      .toLowerCase();
    ...
    return expectedText
      .split(' ')
      .forEach((word) => global.expect(cardText).toContain(word));
  }
  ...
  swipeLeft() {
    ...
    Gestures.swipe(
      ...
      {x: Math.round(carouselRectangles.x + carouselRectangles.width * 0.2), y},
    );
  }
